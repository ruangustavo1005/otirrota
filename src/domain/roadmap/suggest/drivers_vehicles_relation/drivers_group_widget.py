...
from domain.roadmap.suggest.drivers_vehicles_relation.drivers_row_widget import (
    DriversRelationRowWidget,
)


class DriversRelationGroupWidget(QWidget):
    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self.relation_rows: List[DriversRelationRowWidget] = []
        self.setup_ui()
    ...
    def setup_ui(self) -> None:
        ...
        self.group_box = QGroupBox("Relação de Motoristas")
        ...
        driver_label = QLabel("Motorista")
        driver_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header_layout.addWidget(driver_label, 7)
        on_call_label = QLabel("Plantão?")
        on_call_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header_layout.addWidget(on_call_label, 2)
    ...
    def add_relation_row(self, driver: Driver) -> None:
        new_row = DriversRelationRowWidget()
        self.relations_layout.addWidget(new_row)
        self.relation_rows.append(new_row)
        new_row.set_data(driver)

    def get_relations(self) -> List[Tuple[Driver, bool]]:
        relations = []
    ...
    def set_relations(self, relations: List[Driver]) -> None:
        for row in self.relation_rows[:]:
